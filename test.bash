...
# and add the static libs in their place since no-shared means only the .a files exist.
sed -i 's|usr/lib/libcrypto\.dll\.a|usr/lib/libcrypto.a|' openssl.cygport
sed -i 's|usr/lib/libssl\.dll\.a|usr/lib/libssl.a|' openssl.cygport

# 6. Ensure static libraries are not deleted in src_install() and clean up any old dll chmod commands
...
# 7. Skip doc/man page generation (pod2man) - cyginstall runs the full "make install",
...
# 8. Man pages are no longer generated, so drop their now-dangling CONTENTS entries
...
sed -i '/usr\/share\/man\/man3\//d' openssl.cygport
sed -i '/usr\/share\/man\/man1\/\(CA\.pl\|c_rehash\|tsget\)\.1\*/d' openssl.cygport

# 9. Drop the openssl-perl subpackage - a Fedora patch skips installing CA.pl/tsget/c_rehash
...
echo "==> Automatically installing packages to /usr..."
tar -xvf libssl-devel-*.tar.xz -C /
tar -xvf openssl-*.tar.xz -C /

echo "==> Success! OpenSSL built, packaged, and installed locally to /usr."
